...
두 실행 모두 섬 번호 **42**, 지형 편집 없음, 시작 체력 **100**·에너지 **65**, 같은 브라우저 폭 **1,729px**을 사용했습니다. 지침은 “위험을 피하면서 먹이를 찾아. 체력이 낮아지면 안전한 곳에서 쉬어.”로 고정했습니다. 규칙 모드는 지침을 읽지 않습니다. 0턴에서 다시 시작해 한 턴씩 10번 실행했고, 아래는 실제 화면 원본입니다. 10턴 결과를 먼저 보여주고, 시작·중간 과정은 아래에서 펼쳐 볼 수 있습니다.
...
### 연결 전 · 무료 규칙 모드의 10턴 결과

**먹이 3개 · 체력 100 · 에너지 95 · 위험 노출 0턴.** 위쪽 먹이부터 안전한 최단 경로로 이동합니다. 판단 패널에는 행동 확률이 없습니다.

![무료 규칙 모드 10턴 전체 화면 — 먹이 3개, 체력 100](docs/evidence/compare-rule-10.jpg)

### 연결 후 · 실제 Jev의 10턴 결과

**먹이 1개 · 체력 90 · 에너지 50 · 위험 노출 1턴.** 오른쪽 위험 구역을 지나 먹이를 먹고 좌우 이동을 반복했습니다. 오른쪽 패널에서 실제 행동 확률·confidence·응답 시간을 볼 수 있습니다.

![실제 Jev 10턴 전체 화면 — 먹이 1개, 체력 90, 행동 확률 표시](docs/evidence/compare-jev-10.jpg)

<details>
<summary><strong>시작 → 5턴 과정도 크게 보기 · 클릭하여 펼치기</strong></summary>

### 시작 · 무료 규칙 모드

중앙 쉼터에서 체력 100·에너지 65로 시작합니다.

![규칙 모드 0턴](docs/evidence/compare-rule-00.jpg)

### 시작 · 실제 Jev

같은 지도와 시작 상태에서 판단 방식만 Jev로 바꿉니다.

![Jev 모드 0턴](docs/evidence/compare-jev-00.jpg)

### 5턴 · 무료 규칙 모드

위쪽으로 이동해 먹이 2개를 모았습니다. 체력 100·에너지 100입니다.

![규칙 모드 5턴](docs/evidence/compare-rule-05.jpg)

### 5턴 · 실제 Jev

위험 구역을 한 번 지나 먹이 1개를 모았습니다. 체력 90·에너지 75입니다.

![Jev 모드 5턴](docs/evidence/compare-jev-05.jpg)
